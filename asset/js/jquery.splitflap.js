  ...
  lp.setChar = function(char, charFrom, ratio) {
    ...
    if (ratio >= 1) {
      $(this.lowerObject).css({
        "background-position":
          "-" +
          offset * this.settings.charWidth +
          "px -" +
          (this.settings.charHeight >> 1) +
          "px"
      });
      ...
      $(this.upperObject).css({
        "background-position":
          "-" + offsetFrom * this.settings.charWidth + "px 0px"
      });
      $(this.lowerObject).css({
        "background-position":
          "-" +
          offsetFrom * this.settings.charWidth +
          "px -" +
          (this.settings.charHeight >> 1) +
          "px"
      });
    ...
    if (animated) {
      ...
      for (var i = 0, l = this.letters.length; i < l; i++) {
        var al = {
          ratio: 0,
          speed:
            this.settings.speed + Math.random() * this.settings.speedVariation,
  ...
  $.fn.splitFlap = function(options) {
    ...
    return this.each(function() {
      ...
      while (textInit.length < text.length) {
        textInit =
          textInit +
          settings.charsMap.charAt(
            Math.floor(settings.charsMap.length * Math.random())
          );
      }
      ...
      $(this)
        .empty()
        .append(this.splitflap.getDOMObject());
